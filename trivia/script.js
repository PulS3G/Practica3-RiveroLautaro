...
async function inicializarApp() {
  try {
    ...
    document.body.innerHTML =
      "<h1>Error al cargar los datos. No se puede continuar.</h1>";
  }
}

function poblarCategorias() {
  const categorias = [
    ...new Set(todasLasPreguntas.map((p) => p.id.split("_")[0])),
  ];
...
function iniciarJuego() {
  ...
  const filtradas = todasLasPreguntas.filter((p) =>
    p.id.startsWith(catSeleccionada),
  );
...
function mostrarPregunta() {
  ...
  document.getElementById("progress").textContent =
    `Pregunta ${indiceActual + 1} de 5`;
  document.getElementById("question-text").textContent = p.pregunta;

  const opciones = [p.correcta, ...p.incorrectas].sort(
    () => 0.5 - Math.random(),
  );
